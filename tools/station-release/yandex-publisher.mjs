...
async function loadBackup(backupDirectory, channel) {
  ...
  for (const [position, object] of backup.objects.entries()) {
    const backupPath = `object-${position}.bin`;
    if (
      !hasExactKeys(object, ["key", "contentType", "sha256", "backupPath"]) ||
    ...
    objects.push({ key: object.key, contentType: object.contentType, bytes });
  }
  return objects;
}

async function restoreAndVerify(store, objects) {
  for (const object of objects) {
    await store.putMutable(object.key, object.bytes, object.contentType);
  }
  for (const object of objects) {
  ...
  return Object.freeze({
    ...
    async backupMutables({ channel, backupDirectory } = {}) {
      ...
      try {
        objects = await Promise.all(
          expected.map(async ({ key, expectedContentType }) => {
            ...
            return { key, bytes: Buffer.from(object.bytes), contentType: object.contentType };
          }),
        );
      } catch (error) {
        if (error?.message === "complete mutable backup required") throw error;
    ...
    async promote({ tree, channel, backupDirectory } = {}) {
      ...
      try {
        ...
        try {
          await restoreAndVerify(store, backup);
        } catch {
...
async function main() {
  const endpoint = process.env.YANDEX_STATION_RELEASE_ENDPOINT;
  const bucket = process.env.YANDEX_STATION_RELEASE_BUCKET;
  if (endpoint !== YANDEX_S3_ENDPOINT || typeof bucket !== "string") commandInvalid();
  const client = new S3Client({ endpoint, region: "ru-central1", maxAttempts: 3 });
  try {
    const store = createStationObjectStore({ client, bucket, publicBaseUrl: PUBLIC_BASE_URL });
    await runYandexPublisherCli(process.argv.slice(2), {
