...
/**
 * Tenant-owned text, rebuilt per locale; ids, GTINs, dates and counts stay
 * shared so both frames document the same values.
 *
 * `productGroup` is deliberately NOT translated. It is the resolved name of a
 * Chestny Znak product group, and `chz_product_groups.name` is a
 * single-language platform directory the API serves verbatim -- an English
 * frame carrying an English group would document a screen the cabinet cannot
 * render.
 */
const COPY = {
  ru: {
...
function fixtures(locale: AdminLocale) {
  ...
  const PREVIEW_RICH = importPrepareResponseSchema.parse({
    ...
    items: [
      {
        ...BASE_ITEM,
        identity: { ...BASE_ITEM.identity, name: copy.product },
        fields: [
          {
            ...first(BASE_ITEM.fields, "preview field"),
            label: copy.fieldName,
            labelKey: "name",
            before: copy.productShort,
            after: copy.productLong,
          },
          {
            id: id(14),
            label: copy.fieldPrintName,
            labelKey: "print_name",
            before: null,
            after: copy.productShort,
            applicable: true,
            reason: null,
          ...
          {
            id: id(15),
            label: copy.fieldProductGroup,
            labelKey: "chz_product_group_code",
            before: null,
            after: copy.productGroup,
            applicable: true,
            reason: null,
            source: "national_catalog",
            selectedByDefault: true,
  ...
  async function installApi(page: Page, scenario: Scenario) {
    ...
    await page.route(/^http:\/\/127\.0\.0\.1:\d+\/api\//, async (route) => {
      ...
      if (path === "/api/products") {
        ...
        return json(route, { items: linked ? [{ ...productFixture, name: copy.product }] : [] });
      }
...
for (const locale of LOCALES) {
  ...
  test(`[${locale}] the import panel explains a missing Chestny Znak connection`, async ({
    ...
    await expect(page.getByRole("dialog", { name: t("pages.catalog.import.title") })).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "import-unavailable"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] the import panel offers both ways to load products`, async ({ page }) => {
    const unexpected = await installApi(page, "start");
    await openRoute(page, "/catalog/import");
    await expect(
      page.getByRole("button", { name: t("pages.catalog.import.loadOwn") }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: t("pages.catalog.import.loadGtins") }),
    ).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "import-start"));
    expect(unexpected).toEqual([]);
  });
  ...
  async function openSelection(page: Page) {
    await openRoute(page, "/catalog/import");
    await page.getByRole("button", { name: t("pages.catalog.import.loadOwn") }).click();
    await expect(page.getByRole("checkbox", { name: /4006381333931/ })).toBeVisible();
  }

  test(`[${locale}] the selection step lists loaded cards with their match`, async ({ page }) => {
    const unexpected = await installApi(page, "selection");
    await openSelection(page);
    await screenshotFullMain(page, screenshotPath(locale, "import-selection"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] the review step compares current and proposed values`, async ({ page }) => {
    const unexpected = await installApi(page, "review");
    await openSelection(page);
    await page.getByRole("checkbox", { name: /4006381333931/ }).click();
    await page.getByRole("button", { name: t("pages.catalog.import.compare") }).click();
    await expect(page.getByLabel(t("pages.catalog.import.manualName"))).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "import-review"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] a position the cabinet cannot apply says why`, async ({ page }) => {
    const unexpected = await installApi(page, "reviewBlocked");
    await openSelection(page);
    await page.getByRole("checkbox", { name: /4006381333931/ }).click();
    await page.getByRole("button", { name: t("pages.catalog.import.compare") }).click();
    await expect(page.getByLabel(t("pages.catalog.import.manualName"))).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "import-review-blocked"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] the review step offers the National Catalog photo`, async ({ page }) => {
    const unexpected = await installApi(page, "review");
    await openSelection(page);
    await page.getByRole("checkbox", { name: /4006381333931/ }).click();
    await page.getByRole("button", { name: t("pages.catalog.import.compare") }).click();
    // `<fieldset><legend>` for the photo block -- take the section itself: the
    // step is one block of a very tall panel, and a full-page capture would
    // repeat the review frame.
    const photos = page.getByRole("group", { name: t("pages.catalog.import.photo") });
    await expect(photos).toBeVisible();
    await settle(page);
    await photos.screenshot({ path: screenshotPath(locale, "import-photo"), scale: "css" });
    expect(unexpected).toEqual([]);
  });
  ...
  test(`[${locale}] the result step reports the product and its photo separately`, async ({
    page,
  }) => {
    const unexpected = await installApi(page, "result");
    await openRoute(page, resultRoute());
    await expect(
      page.getByRole("button", { name: t("pages.catalog.import.openProduct") }),
    ).toBeEnabled();
    await screenshotFullMain(page, screenshotPath(locale, "import-result"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] an unfinished photo holds back the link to the product`, async ({ page }) => {
    const unexpected = await installApi(page, "resultPhoto");
    await openRoute(page, resultRoute());
    await expect(
      page.getByRole("button", { name: t("pages.catalog.import.openProduct") }),
    ).toBeDisabled();
    await expect(page.getByText(t("pages.catalog.import.waitForPhoto"))).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "import-result-photo"));
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] the link panel shows the bound card and its actions`, async ({ page }) => {
    const unexpected = await installApi(page, "link");
    await openRoute(page, `/catalog/${id(99)}/chz`);
    await expect(page.getByRole("dialog", { name: t("pages.catalog.chz.title") })).toBeVisible();
    await expect(page.getByRole("button", { name: t("pages.catalog.chz.refresh") })).toBeEnabled();
    await screenshotFullMain(page, screenshotPath(locale, "chz-link"), "vertical");
    expect(unexpected).toEqual([]);
  });

  test(`[${locale}] a failed status check keeps the saved details and names the reason`, async ({
    page,
  }) => {
    const unexpected = await installApi(page, "linkError");
    await openRoute(page, `/catalog/${id(99)}/chz`);
    await page.getByRole("button", { name: t("pages.catalog.chz.refresh") }).click();
    await expect(page.getByText(t("pages.catalog.chz.checkError"))).toBeVisible();
    ...
    await expect(page.getByText(t("pages.catalog.chz.errors.card_unavailable"))).toBeVisible();
    await screenshotFullMain(page, screenshotPath(locale, "chz-refresh-error"), "vertical");
    expect(unexpected).toEqual([]);
  });
}
