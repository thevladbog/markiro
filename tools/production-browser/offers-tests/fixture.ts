...
export function makeWorkspace() {
  return offerWorkspaceV2Schema.parse({
    offer: {
      ...
      lines: names.map((nameRu, index) => ({
        ...
        commercialTerms: {
          version: 1,
          subject: "service",
          documentNameRu: nameRu,
          documentNameEn: namesEn[index],
    ...
    parties: {
      seller: {
        ...party("Тестовый поставщик", "234106228141"),
        taxPolicy: {
          kind: "vat",
          regime: "other",
      ...
      sellerBankAccount: {
        id: ID,
        label: "Тестовый счёт",
        settlementAccount: model.seller.bankAccount,
        bic: model.seller.bic,
        bankName: model.seller.bankName,
        correspondentAccount: model.seller.correspondentAccount,
        currency: "RUB",
      },
