...
ALTER TABLE "station_sync_quarantine" ADD CONSTRAINT "station_sync_quarantine_record_kind_check" CHECK ("station_sync_quarantine"."record_kind" IN ('item', 'box', 'exception', 'product_label_event', 'pallet', 'pallet_exception'));--> statement-breakpoint
...
ALTER TABLE "disaggregation_document_lines" ADD CONSTRAINT "disaggregation_document_lines_target_check" CHECK (num_nonnulls("disaggregation_document_lines"."box_id", "disaggregation_document_lines"."pallet_id") <= 1);
--> statement-breakpoint
...
--> statement-breakpoint
-- BACKFILL for the `boxes.disassembly_received_at` column added above.
--
-- `boxes.disassembled_at` is the operator's own device clock (the
