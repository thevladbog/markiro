...
describe.skipIf(!databaseUrl)("default box label template migration", () => {
  ...
  beforeAll(async () => {
    ...
    await rm(join(legacyMigrationsFolder, "0047_product_shelf_life_days.sql"), {
      force: true,
    });
    await rm(join(legacyMigrationsFolder, "0048_default_label_templates.sql"), {
      force: true,
    });
    ...
    journal.entries = journal.entries.filter(
      (entry) =>
        ...
        entry.tag !== "0047_product_shelf_life_days" &&
        entry.tag !== "0048_default_label_templates",
    );
