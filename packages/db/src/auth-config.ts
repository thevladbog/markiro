    ...
    plugins: [
      ...
      apiKey([
        ...
        },
        {
          ...
          // станции (600/мин) здесь не нужен. Но и плагинный дефолт (10 запросов
          // в сутки) не годится: любая небольшая интеграция (постраничная
          // синхронизация каталога и т.п.) упрётся в него за первые минуты.
          // Явно задаём 60 запросов в минуту -- с запасом для обычного
          // использования внешней интеграцией, но заметно ниже станционного
          // потолка; жёстким выключателем остаётся отзыв.
