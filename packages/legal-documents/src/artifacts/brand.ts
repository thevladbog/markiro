...
export function prepareDataMatrixMedia(svg: string): {
  ...
  // the match fails loudly — the regex returns null and the throw below fires.
  // The quiet risk is in the fix, not the break: a relaxed match can select a
  // DIFFERENT path element and yield a subtly wrong barcode, so re-verify a
  // scanned Data Matrix before moving the pin.
