...
export const TENANT_PROCESSING_CONTENT = {
  ru: {
    ...
    summary:
      "Шаблон определяет границы ответственности тенанта как оператора и Маркиро (англ. — Markiro) как лица, обрабатывающего данные по поручению.",
    sections: [
      {
        id: "template-status",
      ...
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Тенант, который определяет цели и способы обработки персональных данных своих работников и иных лиц, является оператором. Маркиро является лицом, осуществляющим обработку персональных данных по поручению оператора, только в пределах документированных инструкций тенанта.",
          },
          {
            kind: "paragraph",
            text: "Роль обработчика не освобождает Маркиро от обязанностей по конфиденциальности, безопасности, соблюдению поручения и содействию оператору.",
          },
        ],
      },
      {
      ...
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Индивидуальный документ должен определить операции, цели, категории субъектов и данных, территорию и срок обработки, порядок доступа, выгрузки, исправления, блокирования, возврата и удаления. Маркиро вправе запросить уточнение противоречивой или технически невыполнимой инструкции и сообщает, если инструкция, по его обоснованному мнению, нарушает применимое право.",
          },
        ],
      },
      {
        id: "processor-duties",
        heading: "5. Обязанности Маркиро как обработчика",
        blocks: [
          {
            kind: "paragraph",
            text: "Маркиро обрабатывает данные только по документированному поручению, обеспечивает конфиденциальность допущенных лиц, применяет соразмерные меры безопасности, сохраняет изоляцию тенантов и предоставляет оператору согласованные сведения, необходимые для подтверждения исполнения поручения.",
          },
          {
            kind: "paragraph",
            text: "Маркиро не использует данные тенанта для рекламы, самостоятельного профилирования работников или целей третьих лиц и не расширяет поручение молчанием либо настройкой интерфейса.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Привлечение субподрядчика допускается на условиях, согласованных с тенантом или предусмотренных договором. Маркиро ведет актуальный состав существенных обработчиков, передает им только необходимый объем и возлагает сопоставимые обязанности по конфиденциальности, безопасности и удалению.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Маркиро без неоправданной задержки сообщает тенанту о подтвержденном инциденте, затрагивающем порученные данные, в объеме доступных проверенных сведений, содействует локализации последствий и последующему уточнению информации.",
          },
          {
            kind: "paragraph",
            text: "С учетом характера обработки Маркиро содействует тенанту в ответах субъектам, оценке требований к безопасности, подтверждении принятых мер и подготовке обязательных уведомлений. Тенант остается ответственным за юридическое решение и ответ как оператор.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "После прекращения поручения Маркиро по выбору, предусмотренному индивидуальным документом, возвращает доступную выгрузку и удаляет порученные данные либо сохраняет строго ограниченную часть, если обязательное требование запрещает немедленное удаление. Резервные копии выводятся из обращения по установленному циклу и не используются для обычной обработки.",
          },
        ],
      },
      {
        id: "independent-processing",
        heading: "9. Самостоятельная обработка Маркиро",
        blocks: [
          {
            kind: "paragraph",
            text: "Маркиро действует как самостоятельный оператор только для отдельно определенных собственных целей, например биллинг и расчеты, безопасность платформы, реагирование на злоупотребления и обязательный учет, если Маркиро самостоятельно определяет цель и основание. Такая обработка не маскируется под поручение и описывается в соответствующей политике или договоре.",
          },
        ],
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Код шаблона: MKR-DPA-01. Редакция: 2026.08/01. Дата вступления в силу шаблона: 15.08.2026. После заполнения и оформления конкретный документ получает уникальный индивидуальный номер; код общего шаблона не подтверждает заключение поручения.",
          },
          {
            kind: "paragraph",
    ...
    sections: [
      ...
      },
      {
        ...
        blocks: [
          {
            kind: "paragraph",
            text: "Template code: MKR-DPA-01. Revision: 2026.08/01. Template effective date: 15 August 2026. A completed and executed document receives a unique individual number; the common template code is not proof of execution.",
          },
          {
            kind: "paragraph",
