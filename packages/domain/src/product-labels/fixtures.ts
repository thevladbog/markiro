...
export function buildProductLabelFixtures() {
  const cases: ProductLabelProjectionCase[] = [
    ...
    run("verification explicitly skipped", "required", [
      prepared(1, ATTEMPT_1, 1, null),
      simple(2, ATTEMPT_1, "sending"),
      simple(3, ATTEMPT_1, "sent"),
      { ...base(4, ATTEMPT_1), kind: "verification_skipped" },
    ]),
    run("a skipped attempt cannot later become verified", "required", [
      prepared(1, ATTEMPT_1, 1, null),
      simple(2, ATTEMPT_1, "sending"),
      simple(3, ATTEMPT_1, "sent"),
      { ...base(4, ATTEMPT_1), kind: "verification_skipped" },
      verified(5, ATTEMPT_1, duplicatePayloadDigest(RAW)),
    ]),
    run("skip does not settle unknown delivery", "required", [
      prepared(1, ATTEMPT_1, 1, null),
      simple(2, ATTEMPT_1, "sending"),
      unknown(3, ATTEMPT_1, "interrupted"),
      { ...base(4, ATTEMPT_1), kind: "verification_skipped" },
    ]),
    run("reprint after skip requires verification again", "required", [
      prepared(1, ATTEMPT_1, 1, null),
      simple(2, ATTEMPT_1, "sending"),
      simple(3, ATTEMPT_1, "sent"),
      { ...base(4, ATTEMPT_1), kind: "verification_skipped" },
      prepared(5, ATTEMPT_2, 2, "lost"),
      simple(6, ATTEMPT_2, "sending"),
      simple(7, ATTEMPT_2, "sent"),
    ]),
