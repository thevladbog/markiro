...
try {
  ...
  for (const [index, sample] of samples.entries()) {
    ...
    cards.push(
      `<article id="${sample.id}"><h2>${escape(sample.category)} · ${sample.widthMm} × ${sample.heightMm} мм</h2><p>${sample.purpose === "box" ? "Коробка · SSCC" : "Дубликат товара · Data Matrix"} · группа ${sample.group}</p><div class="preview"><img src="${sample.preview}" alt="Образец ${escape(sample.category)} ${sample.widthMm} на ${sample.heightMm} мм" /></div><p><a class="download-code" href="${sample.source}" download>Скачать код</a> · <a href="${sample.preview}">Открыть PNG</a></p><details><summary>Код импорта</summary><button type="button">Копировать код</button><textarea readonly aria-label="Код ${escape(sample.id)}" spellcheck="false">${escape(code)}</textarea><span role="status"></span></details></article>`,
    );
  }
} finally {
  await browser.close();
