...
17. Deterministic limit order is all loose lines in request order followed by box lines in request order; a box consumes its full DB-derived bottle count or receives one `over_limit` conflict. Response `acceptedBoxes` is canonical-SSCC sorted so idempotent replay reconstructed from snapshots is byte-stable without a new line-index column. Cost if wrong: a mixed cart near its limit may prefer a loose line over an earlier-scanned box; benefit is explicit semantics for the request's separate arrays and unchanged loose behavior.
