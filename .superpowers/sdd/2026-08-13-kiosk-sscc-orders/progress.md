...
22. Registry cut ownership is `(canonical serverUrl, kioskId, owner, since, until)`, and storage checks the current paired config in the same IndexedDB transaction before beginning, paging, or activating. Cost if wrong: registry operations contend with rare config writes; benefit is that stale refresh work cannot cross a re-pair boundary or activate under another tenant.
23. A full activation must be strictly newer than the active revision; a delta must have the exact active revision as its base. Worker single-flight reduces overlap but is not the correctness boundary. Cost if wrong: a server revision reset on the same installation requires an explicit re-pair/registry reset; benefit is no older full cut can regress a newer active snapshot.
24. Registry freshness is stamped from bootstrap `generatedAt`, never the tablet fetch instant, and interpreted on the same server-corrected clock and warn/block thresholds as the main cache. Cost if wrong: registry freshness follows bootstrap clock quality; benefit is one skew-resistant verdict ready for the touch flow.
25. An untrusted registry page has a one MiB aggregate UTF-8 budget and 1,024-byte per-string budget before durable copying/dedupe, in addition to candidate/member/page caps and UUID/SSCC validation. Cost if wrong: unusually long legitimate member identifiers are rejected; benefit is a bounded parser and IndexedDB allocation boundary.
26. Registry ownership includes a random non-secret credential-generation UUID which rotates with the paired token. Config contains the already-established token; registry metadata contains only the UUID, and every storage CAS plus worker single-flight uses `(serverUrl,kioskId,generation)`. Cost if wrong: same-binding token rotation requires a new full registry refresh; benefit is that a response authenticated by a replaced token can never publish after re-pairing.
...
- Initial review at `739495a5`: CHANGES REQUESTED for four Important findings covering overlapping full cuts, device-clock freshness, unbounded string allocation, and pairing/tenant isolation.
- Fix round RED: 3 files / 12 expected failures with 108 existing passes. Focused final GREEN: box registry/store/sync/pairing 146/146. Full kiosk: 485/485; typecheck and full ESLint passed. Final build/format/diff evidence is recorded in `task-6-report.md`.
- Fix design: installation-bound owner/CAS is enforced inside IndexedDB transactions sharing the config store; worker single-flight is an optimization. Registry metadata uses trusted bootstrap server time; page allocation is bounded before IDB copying; re-pair/revocation clears registry without deleting queue or journal.
- Re-review retained one Important same-binding token-rotation race. Credential fix RED was 1 failed/16 passed; focused GREEN 148/148; final full kiosk 487/487. A random config generation now rotates with the token and participates in every registry CAS and single-flight key; no token or reversible token material is persisted in registry metadata. The exact held-old/new-token winner regression is pinned.
