...
**Only one path actually delivers a top-up today: the shift bundle.** `GET /shifts/:id/bundle` carries a pool for that shift's issuer, which guarantees the device can close boxes for a client whose numbers it has never held, and re-fetches (shift entry, re-entry, app restart) hand back the same block's original bounds plus its consumed-through cursor rather than a fresh range — safe to replay any number of times. A SECOND path was designed alongside it — the sync response carrying further top-ups when the device reports its remaining count has fallen below a threshold, riding the same channel 06b uses for conflicts, so no new endpoint or polling — and the device side of it (`sync.ts`'s `ssccBlock` handling, `SyncState.serialsLeft`) is built and tested. **The owner decided not to build the server side for 06c**: nothing in `station-scans`'s response DTO or ingest emits a top-up over that channel, so it is forward-compatible plumbing today, not a delivered feature. A device that exhausts its block mid-shift has no server-pushed recovery; re-entering the shift (re-fetching the bundle) is what recovers it. The pool itself is independent of shifts: it persists across them and is never surrendered at shift close.

A block is sized so that a device losing its connection at the worst moment still finishes the shift: comfortably more than one shift's boxes, which with ten million serials per extension digit costs nothing. Were the sync-response channel built, its threshold would be set so a top-up arrives while a shift's worth of numbers still remains, rather than shortly before the last one is burned — deferred along with the channel itself.
