...
4. Give organization owners all administrator permissions plus organization membership and administrator management.
5. Deny cabinet access to the Better Auth `member` role and to unknown roles.
6. Keep controller policy expressed in capabilities rather than role names, so later custom roles and per-user grants do not require controller rewrites.
7. Leave device and machine authentication behavior unchanged.
...
- Admin succeeds on operational and administrative routes but cannot perform owner-only member administration.
...
- An owner can perform admin workflows and manage organization membership and administrators.
