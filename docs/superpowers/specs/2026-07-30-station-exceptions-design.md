...
1. **Four actions, not the whole "exceptions" bucket.** Plan 06 named disassemble/replace/reprint/undo. This slice ships **undo, clear, reprint, disassemble** — replacing one code with another inside an already-boxed item, without touching the rest of the box, is deferred (no client need identified yet, and it is a strictly harder variant of the same mechanics below). **Revisited 2026-09-12 when contour 06 closed and the deferral was upheld: replace-in-box is out of the MVP, not merely out of this slice.** See design brief 10 «Out of v1».
