...
`PUT /traceability/profile` — available only in the U.S. deployment, `@RequirePermissions(tenant.settings.manage)`, `@RequireSubscriptionWrite()`. Body `{ code, timeZone?, retentionDays? }`, partial merge like `PUT /org/profile`. The `code` schema contains only the two U.S. profiles. Rules, in the service and inside one transaction:
...
- Settings (`apps/admin/src/pages/settings/OrgProfilePage.tsx`): the U.S. deployment shows a "Regulatory profile" section with only `US_FSMA204_PROCESSOR` and `US_GENERIC_LOT_TRACEABILITY`, a U.S. timezone list, retention days with the 730-day floor, and a confirm dialog on profile change. The Russian deployment does not render this section. Sections `inn`, CHZ category defaults and pickup policy are not registered in the U.S. navigation; GLN, GS1 prefixes, logo and SSCC counters stay because SSCC is the optional case layer.
