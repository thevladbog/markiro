...
| POST   | `/traceability/requests/:id/exports`                 | Freezes the snapshot in the same transaction as the run insert (revision = max + 1), computes `export_ready`, enqueues; idempotent per key. |
...
| OQ-US09-2  | May a package be generated while blocking findings are unacknowledged?                                                                                                                                                           | (a) allowed, run marked `export_ready=false`; (b) hard 409 until resolved/acknowledged                                                                                               | (a): RQ-003 blocks export-ready status, not the drill; the 24 h clock must not be blocked by tooling                                 | yes       |
