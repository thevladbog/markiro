...
- Negative cases from acceptance §2.4: covered product with `unknown` status → `error` finding and US-09 blocks case-ready; master data edited after finalization → identical workbook bytes for the same request revision; duplicate export retry → same hashes, no duplicate artifacts.
