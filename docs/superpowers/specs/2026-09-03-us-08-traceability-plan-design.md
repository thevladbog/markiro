...
CREATE TABLE traceability_plan_versions (
  ...
  CONSTRAINT traceability_plan_versions_status_consistency CHECK (
    (status = 'draft' AND effective_at IS NULL AND approved_at IS NULL AND pdf_object_key IS NULL)
    OR (status = 'effective' AND effective_at IS NOT NULL AND approved_by_user_id IS NOT NULL
        AND config_snapshot IS NOT NULL AND config_digest IS NOT NULL
        AND pdf_object_key IS NOT NULL AND pdf_sha256 IS NOT NULL AND superseded_at IS NULL)
    OR (status = 'superseded' AND effective_at IS NOT NULL AND superseded_at IS NOT NULL
...
| POST   | `/traceability/plans/:id/approve`  | Body `{ idempotencyKey }`. One transaction: validate, freeze snapshot + digest, render PDF, `putVerified`, mark previous effective `superseded`, set dates. |
...
Approval is synchronous (one page-set PDF renders in well under a second in the existing renderers); the idempotency key is stored on the row and a repeat with the same key returns the same effective version (OQ-US08-1). Object key: `tenants/{tenantId}/traceability/plans/{versionId}/traceability-plan-v{N}.pdf`. Renderer version constant `traceability-plan-pdf-v1` is written to `pdf_renderer_version` and printed in the footer.
...
| ---------- | ------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------- |
| OQ-US08-1  | Render the PDF synchronously inside the approve request or through a pg-boss job like shift exports?    | (a) synchronous in one transaction; (b) job with pending/ready states as `invoice_documents`                    | (a): a few pages, no external inputs, and approval must return an effective version atomically | no        |
