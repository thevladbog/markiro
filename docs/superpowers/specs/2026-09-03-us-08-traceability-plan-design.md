...
- `docs/superpowers/specs/2026-09-03-us-00-regulatory-profile-design.md` — `traceability_profiles.retention_years`, `RequireTraceabilityProfile`, U.S. capabilities
...
- `defaultReviewDueAt(effectiveAt)` = `effectiveAt + 365 days` (PLN-009, optional operational reminder, not record retention).
...
Approval renders outside the publication transaction, with attempt-isolated storage:

1. Resolve idempotency against this version and payload; a reused key with a different payload conflicts.
2. Freeze draft revision, config digest, approver and one effective timestamp; render those exact inputs. Upload with verification to a unique attempt-scoped key.
3. In a short transaction, lock the tenant plan lineage and draft, recheck both draft and config revisions, then supersede the prior effective version and publish the winning key/hash and the frozen timestamp. A losing concurrent attempt returns the existing idempotent result or a conflict.
4. Cleanup may delete only that attempt's unreferenced object. Never overwrite or delete another attempt's published PDF.

Object keys: `tenants/{tenantId}/traceability/plans/{versionId}/attempt-{attemptId}/traceability-plan-v{N}.pdf`. Retain published bytes unchanged. Tests cover simultaneous approvals, a narrative edit during rendering, lost responses and storage failures.
...
- i18n keys `pages.traceability.plans.*` (US-01 convention) added to both `en.json` and `es.json` (edition-aware EN/ES key coverage). Every key is checked by the US-00 prohibited-wording content test.
...
| OQ-US08-5  | Retention materialization for PLN-007/REG-009                                                           | (a) read `traceability_profiles.retention_years` (US-00, floor 2); (b) per-version `retention_until`; (c) both              | Resolved: calendar-year retention with a separate floor from plan supersession; no P0 purge. See [MVP contract](../../us/mvp-contract.md). | no        |
