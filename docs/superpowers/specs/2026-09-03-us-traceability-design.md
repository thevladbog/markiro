# Markiro U.S. Traceability — bounded context and tenant profile — Design Spec
...
- `docs/us/requirements.md` — 171 normative requirements (132 P0).
...
Markiro today is a Russian-market production and labeling platform (Chestny ZNAK, EGAIS, CommerceML). The U.S. product must reuse its tenancy, RBAC, audit, offline Station, label engine and evidence tooling while adding FSMA 204 concepts that have no Russian counterpart: traceability lots with TLC and TLC source, three critical tracking events, lot genealogy, a versioned Traceability Plan, a 24-hour trace request and an FDA-aligned sortable workbook. The alternative of a separate repository was considered and rejected.

## Decision 1: one repository, new bounded context, no fork

U.S. functionality is added as a `traceability` bounded context in this monorepo, following MUS-001 §1 and §3.2 and the non-negotiable boundaries of the agent master prompt.

Why:

- The NIW case exhibit must show a clear link from the working Markiro foundation to the U.S. adaptation (spec §13.1). A separate repository weakens that link.
- Tenancy, composite tenant FKs, capability RBAC with denial tests, exact audit, additive migrations, `ru`/`en` locales in admin and station, react-pdf and DOCX generation, and `tools/evidence-package` already exist here. Re-creating or extracting them would roughly double the 115–156 hour estimate.
...
- The public case-ready release is a tag of this repository; a synthetic U.S. tenant, not a separate codebase, provides the clean demo surface.
...
## Decision 2: tenant regulatory profile as the gating mechanism
...
US-00 introduces a tenant-level regulatory profile with three codes:

| Code                          | Behavior                                                                                              |
| ----------------------------- | ----------------------------------------------------------------------------------------------------- |
| `RU_CHZ`                      | Default for every existing tenant. Behavior unchanged; Russian fields and documents stay visible.     |
...
- Backfill sets `RU_CHZ` for all existing tenants in the same migration so no tenant is ever profile-less.
