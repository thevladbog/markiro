...
Additive migration `NNNN_us_traceability_transformation.sql` (after US-03; choose the next free migration number at implementation) in `packages/db/src/schema/traceability.ts`.
...
| `event_date` on the common shell                          | `date` NOT NULL at finalization (the KDE "transformation completed date", TRN-007)                                                                                 |
| `completed_at` (optional), `event_time_zone` on the shell | `timestamptz` + IANA text, both set together; zone copied from `org_profiles.time_zone` when the form is saved                                                     |
...
| `lot_id`                           | `uuid` nullable until finalized; composite FK to `traceability_lots(tenant_id, id)`; unique `(tenant_id, event_id, lot_id)` partial |
| `product_snapshot`                 | jsonb at finalization                                                                                                               |

`trace_lot_boxes` (P0 server-side: LOT-010, TRN-010 — bridge, no change to `boxes`):
...
1. Run the validator; on failure return 409 `event_incomplete` with the report (no partial writes).
...
3. On initial finalization create the `traceability_lots` row: `assignment_basis = transformation`, `source_location_id = event.location_id`, `status = active`, `production_date = line.production_date ?? completed_on`, `origin_event_id = event.id`; a LOT-007 duplicate maps to 409 `TLC_DUPLICATE_AT_SOURCE`. Set `line.lot_id` (LOT-006).
4. Insert `lot_genealogy_edges` from `planGenealogyEdges`; run `wouldCreateCycle` (US-02) defensively.
5. Compute operational effects against the state excluding the preceding revision. Do not consume already-consumed inputs twice; preserve quarantine/recall holds and record the exact recalculation.
...
`apps/admin/src/pages/traceability/transformations/` (i18n `pages.traceability.transformations.*`, en/es):
...
- API e2e (`apps/api/test/traceability-transformations.e2e.test.ts`), golden case from `docs/us/demo-scenario.md`: inputs `OSS-260914-A1` 500 lb and `OSS-260914-A2` 500 lb (regulated, imported lots from a seeded receiving), reason `commingling_and_repacking`, completed 2026-09-15 in `America/Los_Angeles`, documents `WO-2026-0915-APPLECUP` and `BATCH-2026-0915-01`, output `NRF-260915-APL01` 100 case (900 lb equivalent recorded as `notes`/waste 100 lb operational), no shift or boxes → finalize 200; assert 2 edges, both inputs `consumed`, output lot `active` with source North River, zero required bridge rows, audit rows; second finalize identical body; PATCH after finalize → 409; amend → new draft revision, finalize it → previous `amended`, previous snapshot bytes unchanged, edges of both revisions present; void with downstream shipping → 409; void without → status `void`, reason stored; P0 server-bridge tests separately: shift of another tenant → 404; active shift → 409; box already linked → 409; unknown coverage on output product → 409 event_incomplete with issue code; product master renamed after finalize → detail snapshot unchanged (acceptance §2.4).
...
| OQ-US04-7  | Amendment and edges: which revision's edges does a trace use, and how is the old revision marked?                                       | keep old edges, filter by event status in US-06; delete old edges; flag column on edges                                                                                      | Resolved: keep historical edges, atomically switch active revision effects; pinned requests retain their edge set. See [MVP contract](../../us/mvp-contract.md).                                                                      | no        |
...
## P0 and amendment corrections

Server box-link routes/schema are P0 after MUS-CR-001; Station-dependent shift selection/automatic closure remains deferred. Keep a core finalization test with two input lots and a 100-case output without boxes; add the P0 synthetic server-bridge scenario with 100 cases, audited link/unlink, replay/conflict and tenant denial. Any supporting synthetic shift rows do not expose a Station/shift workflow. `event_date` exists only on the common event shell; do not add a second completion-date column.

On amendment, reuse existing output lot IDs before duplicate-TLC checks; reject identity/topology changes with finalized downstream dependencies. Validate input availability with the old revision effects removed. All active edge and balance effects change atomically; old revision edges remain available to pinned requests. The `regulated` flag is validated against reviewed product coverage, not an operator bypass. See the shared contract for concurrency tests.
