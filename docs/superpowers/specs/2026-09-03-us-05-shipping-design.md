...
- `docs/superpowers/specs/2026-09-03-us-02-product-profiles-and-lots-design.md` — `traceability_lots`, status enum and `assertLotTransition` (LOT-009), `buildProductSnapshot`, `UOM_CODES_V1`.
...
- `docs/superpowers/specs/2026-09-03-us-01-parties-locations-design.md` — location roles (LOC-006), `buildLocationDescriptionSnapshot`, same-address detection (LOC-007) (planned sibling spec).
...
| product_snapshot    | jsonb NULL; US-02 `buildProductSnapshot` of the lot's product at finalization                                              |
| notes               | text NULL                                                                                                                  |
...
- `balance.ts`: `computeLotBalance({ origin, shipments, transformations })` → `{ originQuantity, unit, shipped, consumed, remaining, comparable, mismatchedLines[] }`, pure and deterministic; `comparable = false` and `remaining = null` as soon as one line's UOM differs from `unit` (no conversion table exists, NFR-009), `mismatchedLines[]` names them.
- `lot-status.ts`: `deriveLotStatusAfterShipping(lot, balance)` → the current status unchanged when `balance.comparable` is false; `shipped` when `remaining <= 0` in origin UOM; otherwise unchanged (`active`); on void/amend the same function recomputes and may return `active` again. Dependency on US-02: `assertLotTransition` must add the row `shipped → active` restricted to the operation context `system:shipping_recalculation` (never offered on `POST /traceability/lots/:id/status`, OQ-US05-5); the resulting `traceability.lot.status_changed` audit row is written with `actor_user_id = NULL` (system) and `after = { status: "active", reason: "system:shipping_recalculation", eventId, eventRevision }`, while the user who voided or amended is on the shipping event's own audit row (`after.lotStatusChanges`). A derived label `partially_shipped` is computed for UI/export from `status = active && shipped > 0` and is not stored (OQ-US05-1).
...
- Domain: `validateShippingCompleteness` per issue code; `assertShippingLineHasNoLotIdentity` rejects every identity key; `computeLotBalance` with partial shipments, mixed UOM → `comparable: false` / `remaining: null`, void/amend exclusion; `deriveLotStatusAfterShipping` transitions `active → shipped → active` (void) and leaves the status unchanged on an unknown balance; same-address normalization (P1).
