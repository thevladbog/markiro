...
**Amendment (post-review, correcting the note above):** the note's own reasoning ("the caller surfaces it verbatim") was never true of the implementation it described — mapping 403 to `{ status: "unauthorized" }` (no `code`, no `message`) discards ЧЗ's text entirely and sends a terminal, operator-actionable contract refusal down `giveUpOnToken`'s token-retry path instead of surfacing it. 401 alone now maps to `unauthorized`; 403 falls through to the generic 4xx branch just below it, which is already `rejected` and already carries the message — no new branch needed, only removing 403 from the first check. Every runner call site that used to receive `unauthorized` for a 403 already handles `rejected` as an ordinary non-`ok` result (`createTask`'s explicit `rejected` case fails the run with `CHZ_TASK_REJECTED` and the ЧЗ message; the poll/download paths log and continue or fail with the message the same way), so this is a client-only change. `chz-true-api-client.test.ts` gained a case for a 403 with an `error_message`, asserting `{ status: "rejected", code: "403", message: ... }`; `chz-export-runner.service.test.ts` gained one showing a 403 rejection lands the run `failed` with the ЧЗ text intact.
...
**Amendment (final-review fix, token outcomes are not all equally retryable):** step 3's original text below treats every non-`ok` `getActiveToken` result the same — a wait while `attempt.retryCount < attempt.retryLimit`. That is wrong for two of the five outcomes: `unconfigured` (no encryption key configured) and `undecryptable` (rotated key or corrupted ciphertext) cannot self-heal on their own the way `missing` and `expired` can — those two are resolved by the signer agent's own scheduler within about fifteen minutes, which is what the wait is for. Without a distinction, an order placed against an undecryptable token would sit `queued`, reporting «В очереди», for the full `MAX_EXPORT_PASSES` budget (240 passes, roughly two hours at the worker's 30-second cadence) before finally failing with `CHZ_TOKEN_UNAVAILABLE` — a failure that was knowable on pass one. `giveUpOnToken` now branches on the two never-self-healing reasons explicitly (`reason === "unconfigured" || reason === "undecryptable"`) and fails every `queued` run immediately when either applies, regardless of `attempt.retryCount`; `missing`, `expired`, and the literal `"unauthorized"` passed in from a mid-pass 401/403 keep the original wait-and-retry behaviour. Branching on the two terminal reasons (not the retryable ones) means an unrecognised sixth `ChzTokenResult` variant, should one ever be added, falls on the safe, retryable side instead of failing outright.
...
2. Timeout sweep, before the token gate and before any network call: fail any `ordered` **or** `ready` run whose `orderedAt` is older than `const MAX_ORDER_WAIT_MS = 6 * 60 * 60_000;` (six hours — comfortably longer than any legitimate dispenser task, shorter than the token's ten-hour life) with `CHZ_TASK_TIMED_OUT`. `markOrdered` stamps `orderedAt` and `markReady` preserves it, so this needs no token and costs no request — it runs unconditionally on every pass, which is what stops an order from being an immortal job when the tenant's agent never refreshes its token (a `queued` run would otherwise be the only thing `giveUpOnToken` can fail) or when a `ready` run never becomes downloadable (`importReadyRuns` logging `unavailable` and moving on every pass, forever). If every run is now terminal, return `{ finished: true }` without ever requesting a token.
3. `getActiveToken`. On anything but `ok`: if `attempt.retryCount < attempt.retryLimit`, journal a warning and return `{ finished: false }` so the job retries; otherwise fail only the still-`queued` runs with `CHZ_TOKEN_UNAVAILABLE` and return `{ finished: <every run is imported or failed> }`. A `queued` run cannot make progress without a token at all; an `ordered` or `ready` run has already paid ЧЗ's finite daily quota for a task or a report, so it is left non-terminal — a later pass, once the tenant's agent refreshes the token, can still poll or download it, and the timeout sweep in step 2 is what keeps that from being forever.
4. Read the inventory's `participantInn` (`org_profiles.inn`), the product's `chzProductGroupCode` and its `gtin14` once, for all six.
5. Before claiming, fail any `queued` run whose `attempts` has already reached `const MAX_CREATE_ATTEMPTS = 10;` with `CHZ_CREATE_ATTEMPTS_EXHAUSTED`, without calling `createDispenserTask`. `attempts` survives an operator retry on purpose (`ChzExportsService.retry()` preserves it), so a run that is retried while already at the cap must fail fast here rather than loop through claim/create again.
6. For each remaining `queued` run, claim it before the request goes out. The claim is what serialises two workers and what makes a lost create response recoverable, so it is one conditional statement:
...
On `ok`, transition to `ordered` with the id; on `rejected`, `failed` with `CHZ_TASK_REJECTED` and the ЧЗ message; on `unauthorized`, treat as the token case in step 3; on `unavailable`, leave it `queued` for the next pass.

7. Collect every remaining `ordered` run's `dispenserTaskId` and call `listDispenserResults` **once**. For each result reporting completion with a `resultId`, transition to `ready`. A result in a failed/cancelled ЧЗ state becomes `failed` with `CHZ_TASK_FAILED`. (A run past the timeout deadline is never seen here — step 2 already failed it before this pass reached the batch poll.)
...
- Produces: `RUN_CHZ_EXPORT_QUEUE = "run-chz-export"` exported from `jobs.module.ts`, and `PgBossService.enqueueChzExportOrder(tenantId: string, inventoryId: string): Promise<string | null>` used by Task 4 (`null` means pg-boss deduped the send against an already-pending job for the order -- see the singleton-key amendment below -- which is not a failure).
...
**Amendment (post-review, restart safety):** `MAX_EXPORT_PASSES` bounds one continuous chain of pg-boss jobs, not the order itself — the count only ever lives in a job's payload. pg-boss jobs, unlike that in-memory count, survive a restart on their own, so `reconcileUnfinishedChzExports` was found unconditionally sending a fresh job (with no `pass`, defaulting to 0) for every order with a non-terminal run at boot, while the previous chain's correctly-numbered next-pass job was typically still sitting there, `created`. Two consequences: every restart raced a redundant pass-0 chain against the surviving one, and — because the budget is per chain, not per order — an order whose restarts came faster than its 240-pass budget could burn would dodge the cap indefinitely, reopening exactly the unbounded path the pass counter exists to close.

The fix keys every send to `RUN_CHZ_EXPORT_QUEUE` — the worker's own chain re-enqueue, `enqueueChzExportOrder`'s initial send, and `reconcileUnfinishedChzExports`'s boot send — on `` `${tenantId}:${inventoryId}` `` (`chzExportSingletonKey`), and gives the queue pg-boss's `stately` policy. Verified against the installed `pg-boss@12.27.0` package (`node_modules/pg-boss/dist/plans.js`): `stately`'s uniqueness (`job_i3`) is `UNIQUE (name, state, COALESCE(singleton_key, '')) WHERE state <= 'active'`, i.e. per `(order, state)` among `created`/`retry`/`active`, not just per order. That is deliberate, not a weaker fallback: keying on `state` too is what lets the worker's own re-enqueue use the same mechanism without deadlocking on itself (the sending job is still `active` while its `created` successor is inserted — different states, no conflict), while still refusing a second `created` job for an order whose previous chain is waiting out its `startAfter` delay — the exact case that made restarts dangerous. A deduped `send()` resolves to `null` (pg-boss's `createJob`, `dist/manager.js`, returns `null` after `ON CONFLICT DO NOTHING` yields no row — it never throws), and all three send sites treat that as an expected "a chain is already pending" outcome, not an enqueue failure. The one gap this doesn't close: a restart landing while a pass is actively _running_ (no `created` row to conflict with yet) can still start a second pass-0 chain — but that is self-limiting, since whichever chain re-enqueues its next `created` pass first wins the order's slot and the other's next attempt hits the same dedup, so a mistimed restart costs an order at most one extra pass, never a repeatedly reset budget. (A fully airtight alternative — an explicit "does a pending job already exist" check before every boot send, with no reliance on queue policy — was considered and rejected: it would need raw SQL against pg-boss's internal, queue-specific job table, which is not part of its public API and would be brittle to depend on.)

**Amendment (post-review, queue policy is load-bearing and pg-boss won't change it in place):** the `stately` policy above is not cosmetic — it is what makes `chzExportSingletonKey` actually dedup, via the `job_i3` unique index. But pg-boss v12's `create_queue()` SQL function only sets a queue row's `policy` the first time that queue name is inserted (`ON CONFLICT DO NOTHING`), and `updateQueue` has no support for changing an existing queue's `policy` either. Every job insert reads `policy` from the _current_ `queue.policy` row at send time (`dist/plans.js`'s `insertJobs`), so a `run-chz-export` queue that was ever created under `standard` (pg-boss's own default) stays on `standard` forever — `createQueue(RUN_CHZ_EXPORT_QUEUE, { policy: "stately", ... })` on every later boot is silently a no-op, `singletonKey` keeps being accepted on every `send`, and the dedup this whole task exists for never fires. No error, no warning.

This was not hypothetical: this repo's dev database had `run-chz-export` sitting under `policy = 'standard'` (left over from an earlier version of this branch) even after the `stately` policy was added to the `createQueue` call. It was fixed by hand — confirming zero rows in `pgboss.job` for the queue first, then `delete from pgboss.queue where name = 'run-chz-export';` and letting the next boot's `createQueue` recreate the row under `stately`.

Because pg-boss gives no signal when this happens, `jobs.module.ts` now asserts it at boot instead of trusting `createQueue`'s silence: right after `createQueue(RUN_CHZ_EXPORT_QUEUE, ...)`, `assertChzExportQueuePolicy` queries `pgboss.queue` directly (`select policy from pgboss.queue where name = $1`) and throws if the actual policy isn't `"stately"`, failing `onModuleInit` (and therefore app startup) with a message naming the fix. Failing loudly rather than auto-repairing is deliberate: dropping and recreating the queue row is only safe when it has no jobs, and a boot path has no business making that judgment call or deleting rows out from under an in-flight export chain. The only environments that can currently have the wrong policy are ones that already ran this unreleased branch, so a hard boot failure there is a one-time, actionable cost — cheaper than leaving every future environment exposed to the same silent trap.

Proving this with a test needed more than the existing mock-level suite: `chz-export-job.test.ts` mocks `pg-boss` entirely, so its assertions only show that `jobs.module.ts` _calls_ `createQueue`/`send` with the right arguments, never whether pg-boss itself would reject a duplicate — the same assertions pass identically whether the queue is `standard` or `stately`. `apps/api/test/chz-export-queue-policy.integration.test.ts` closes that gap: it runs the real `pg-boss` package against a real scratch database with no mock, and reproduces the exact bug — a queue created under `standard`, then `createQueue`d again with `policy: "stately"`, still reports `policy = 'standard'` and still accepts a duplicate `singletonKey` send — alongside the positive case (a queue created fresh under `stately` rejects the duplicate). `chz-export-job.test.ts` also gained a mock-level test asserting `onModuleInit` rejects when the policy probe reports anything other than `"stately"`.

**Amendment (post-review, a thrown pass could strand an order with no way back):** `ChzExportRunnerService.run` (Task 5) rethrows anything that is not a `ChzUnauthorizedError`. Before this fix the worker just `await`ed it directly, so a thrown pass propagated out of the `boss.work` callback: pg-boss retries the _job_ itself (this queue's own `retryLimit: 5`), and — because the throw happens before the `if (!finished)` branch — no `startAfter` successor is ever sent. Once those five job-level retries were spent, nothing would ever run another pass for that order: the runs sit non-terminal forever, `ChzExportsService.retry()` refuses them (it requires `state = "failed"`), nothing else sweeps them (the six-hour deadline only fires on a pass that actually runs), and boot reconciliation is the only way back — capped at `SHIFT_EXPORT_RECONCILE_LIMIT` orders per boot, so not a guarantee. The worker now wraps the `run()` call in `try`/`catch`: while `job.retryCount < job.retryLimit` it rethrows unchanged, so pg-boss's own retry/backoff behaves exactly as before; once `job.retryCount >= job.retryLimit` (this was the job's last attempt) it instead calls a new `ChzExportRunnerService.abandonAfterJobRetriesExhausted(tenantId, inventoryId)`, which reuses the same `failNonTerminal` helper `giveUpOnToken` already uses rather than a second failure path, and fails every remaining non-terminal run with a new code, `CHZ_JOB_RETRIES_EXHAUSTED` (added to `CHZ_EXPORT_SAFE_ERROR_CODES`, with matching `error.<CODE>` copy in both `ru.json` and `en.json` telling the operator to retry or upload manually). `chz-export-job.test.ts` gained a DB-backed test driving the real captured handler with a real `ChzExportRunnerService` whose `run()` is forced to always reject, proving both halves: the error still propagates while budget remains, and the runs end up `failed` once it doesn't.
...
**Amendment (post-review, this suite could send a real request to ЧЗ production from CI):** `chz-exports.e2e.test.ts` boots the real `AppModule.forRoot`, which starts the real `PgBossService`; `ChzExportsService.order` enqueues the real `run-chz-export` job onto it. The fixture seeds a token but no ЧЗ integration channel row, so `ChzTokenService` falls back to the default `production` environment — meaning the real runner, picked up by the real worker mid-test, could fire a real HTTPS request carrying the fixture's fake bearer at `https://markirovka.crpt.ru/api/v3/true-api`. (It also made the `retry` test's assertion that the `RETIRED` run is still `queued` timing-dependent, since the real runner could claim and advance it first.) The fix overrides `TrueApiClient` in the testing module (`Test.createTestingModule(...).overrideProvider(TrueApiClient).useValue(...)`) with a fake that answers every method `unavailable` — chosen over disabling `PgBossService` because it composes with how the suite already builds its app via `AppModule.forRoot`, and it keeps runs deterministically `queued` (fixing the `retry` test's timing dependency as a side effect) rather than risking an unhandled path. The suite also spies on `globalThis.fetch` and asserts zero calls in `afterAll` — a positive proof that nothing escaped, not just that the suite stopped flaking, since the real client's dependencies bind that exact function. A second, quieter half of the same exposure: rows this suite orders never advance past `queued` (the fake always answers `unavailable`), Postgres is shared across every e2e file (`fileParallelism: false`, one database), and every other file's own `AppModule.forRoot` boot runs `reconcileUnfinishedChzExports`, which re-enqueues _any_ non-terminal `chz_export_runs` row it finds against the _real_ `TrueApiClient` those files don't override. Left behind, this suite's own rows would reopen the exact exposure the override closes, just from a different file's boot — so `afterAll` now deletes this suite's `chz_export_runs` rows (tracked by tenant id) before `app.close()` tears down the pool. The other ЧЗ suites (`chz-product-groups.e2e.test.ts`, `signer-agent-tasks.e2e.test.ts`, `signer-scheduler.e2e.test.ts`) were checked for the same exposure: none of them create `chz_export_runs` rows or call `ChzExportsService.order`/`.retry`/`PgBossService.enqueueChzExportOrder`, so boot reconciliation in those files has nothing of this shape to find, and `TrueApiClient` is otherwise reachable from no other code path in the API.
...
**Amendment (post-review, the retry button ignored `canMutate`):** `ChzExportRunStatus` never received `canMutate` at all, unlike `ChzExportOrderButton` and the upload zones right next to it in `InventoryDetailPage.tsx`'s `ExportsStep` — a read-only user (no `OPERATIONS_WRITE`) saw a working-looking «Повторить» button on any failed run. The server refuses the mutation, and the component never renders `retry.isError`, so the click just silently did nothing. `ChzExportRunStatus` now takes `canMutate: boolean`, threaded from the same `canMutate` `ExportsStep` already passes to `ChzExportOrderButton` and the `FileDropZone`s, and the retry button's render condition is now `!canMutate || run.errorCode === RETRY_EXHAUSTED_ERROR_CODE ? null : <Button .../>`. `inventory-chz-exports.test.tsx` gained a test rendering a failed run for a read-only access document (`capabilities: [CABINET_CAPABILITY.OPERATIONS_READ]`) and asserting no «Повторить» button exists.

**Amendment (post-review, the first poll could leave the upload slots stale forever):** `useChzExportState`'s `queryFn` (Step 3 below) invalidates the inventory-detail query when `importedRunCount(next) > importedRunCount(previous)`, but skipped the comparison entirely when `previous` was `undefined` — the very first fetch this query ever makes. `useInventory` and this query are independent and unordered, so if `useInventory` happens to resolve before `ExportsStep` mounts and that very first `chz-exports` response already carries an imported run — with every run terminal, so `refetchInterval` returns `false` and polling never runs again — nothing would ever invalidate `useInventory` for this order, and the six upload slots would render the stale `imports` list forever, not just until the next poll. The fix keeps the growth check for a defined `previous`, but on the first fetch (`previous === undefined`) invalidates whenever `importedRunCount(next) > 0` instead of skipping unconditionally. `inventory-chz-exports.test.tsx` gained a test whose very first `chz-exports` stub response already contains an imported run and asserts the inventory-detail query still gets refetched.
...
**Amendment (post-review, correcting the second bullet above):** that bullet's claim — that a header mismatch is fixed in "the one adapter in `chz-export-runner.service.ts`" — is wrong. That adapter only names the synthesised file (`chz-${status}-${taskId}.zip`), which selects the container kind `importEvidence` unpacks; it does not touch the archive's bytes, so renaming it again would just repeat the same parser diagnostic. The shared parser must stay untouched either way — manual upload depends on it byte-for-byte, so a fix that changed what it accepts would silently change that path too. If the sandbox proves a real format difference, the fix belongs in that same adapter, but as a normalise-or-repackage step on the archive bytes before handing them to `importEvidence`, not as a rename. `docs/runbooks/signer-agent-manual-e2e.md`'s own "Is the dispenser's CSV byte-identical" section has been corrected to say this.
