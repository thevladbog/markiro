...
> **Correction (final review):** this plan's snippets below key `ssccCounters`/`ssccBlocks` by `issuerGln` (a 13-digit GLN) via `resolveIssuerGln`. The implementation was corrected mid-execution to key by the derived 9-digit **issuer prefix** instead (`issuerPrefix`/`resolveIssuerPrefix`) — one GS1 member commonly holds several GLNs sharing one prefix, and GLN-keying would give each its own counter, letting `buildSscc` produce the same SSCC from two independently-numbered ranges. Only `docs/superpowers/specs/2026-07-29-aggregation-boxes-design.md` was corrected at the time (see its "The number space" section, in particular the paragraph beginning "The counter is therefore keyed `(tenant, issuer prefix, extension digit)`"); this plan document's literal snippets were not. **Do not follow this plan's `issuerGln`/`resolveIssuerGln` snippets literally** (e.g. when implementing pallets in slice 06d) — key by the derived 9-digit prefix, matching `packages/db/src/schema/platform.ts` and `apps/api/src/modules/sscc/sscc.service.ts` as actually shipped.
