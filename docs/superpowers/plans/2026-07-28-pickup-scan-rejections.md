...
- `POST /kiosk/orders`' request and response contract does not change **at plan-writing time**. `OrderConflict["reason"]` in `apps/api/src/modules/pickup-orders/dto.ts` keeps its six members (still true). The status code did change after this plan was executed: the unrecognised-badge branch of step 2 now answers **`422`**, not `401` (`fix(api): a bad badge is not a bad device`). `401` on that route is reserved for the DEVICE token — the kiosk sync worker deliberately excludes it from `TERMINAL_STATUSES` (`apps/kiosk/src/sync/worker.ts`) so a revoked kiosk goes back to pairing with its queue intact, which left an order naming a deleted employee both undeliverable and unquarantinable, parked at the head of the offline queue forever. Every `401` below that concerns an **unrecognised badge** — Task 1's index comments, Task 2's test names, `.expect(401)` calls and step-2 snippet — is the historical instruction, not what shipped; read it for intent, not for the exact status. The `401`s about `TenantGuard`/`SessionOnlyGuard` (Task 5) and the kiosk pairing rate limit (Task 4) are unaffected and still correct.
