...
План выполнен, но в четырёх местах ревью показало, что его текст ниже неверен.
**Авторитетен код, а не эти шаги.** Раздел добавлен постфактум, чтобы шаги ниже
не увели будущего читателя не туда.

1. **Проверку даты нужно перезапускать после реконсиляции.** Шаг Task 2
   утверждает обратное («проверку даты повторять не нужно»). Это неверно:
   реконсиляция может переклассифицировать код с `duplicate` на `expected`,
   освободив неудачный claim, и такой скан проскакивал мимо проверки с чужой
   датой. В коде осталась одна проверка — после реконсиляции.
2. **Проверка выполняется только для события без резервирования.** Она стоит
   после разбора `existingEvent`, иначе повтор с тем же `eventId` возвращал бы
   `date-mismatch` вместо durable-результата, а pending-событие переставало
   восстанавливаться через `reconcilePendingInventoryEvents`. Дата уже
   проштампована на строке события при резервировании и не пересматривается.
3. **В перекладке проверка использует `eligible`, а не `classification.kind ===
"expected"`** (шаг Task 4). Условие обязано совпадать с условием добавления
   в короб, иначе бутылка, отсканированная повторно после «удалить последнюю»
   (`facts.reattachAllowed`), попадала в короб без проверки даты.
4. **Диалоги расхождения недоступны на время записи** (`backDisabled={dialogBusy}`
   плюс `disabled` на действии). Разметка в шагах Task 3 и Task 5 этого не
   содержит и дублирует подпись `backLabel` отдельной кнопкой в теле диалога —
   два контрола с одним доступным именем. Без блокировки касание «пропустить»
   во время записи возвращало в очередь скан, который оператор только что
   пропустил.

**Атомарность записи даты сознательно не обеспечивается.** Записи
`setInventoryProductionDate` не объединены в транзакцию с журнальной вставкой:
`tauri-plugin-sql` раздаёт запросы из пула соединений, поэтому `BEGIN`/`COMMIT`
отдельными вызовами ничего не группируют — то же ограничение задокументировано
у `upsertBundle` в `mirror.ts` и у `recordScan` в `journal.ts`. Компенсирующая
запись вместо транзакции рассматривалась и отвергнута: она перезаписывает дату
вслепую и способна затереть более позднее легитимное изменение. Порядок
«сначала дата» выбран осознанно, компромисс описан комментарием в
`inventory-repacking.ts`.
