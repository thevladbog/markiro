...
- Brand background contains only the 24 px grid, representative code string, local date, and lower metadata; no DataMatrix, large square, crosshair, scan line, color or background gradient, photo, or decorative animation. The two `background-image: linear-gradient(...)` declarations that draw the 24 px grid are structural and permitted. Beyond those grid-drawing gradients, the grid-opacity mask is the sole decorative gradient exception: `mask-image: linear-gradient(135deg, transparent 3%, black 34%, black 100%)`.
...
Create `login.css` with the exact structural values below, then add token-based hover/focus rules for the two header buttons and password button. The grid-drawing `background-image: linear-gradient(...)` declarations below are structural and permitted. Beyond those, the diagonal `mask-image: linear-gradient(135deg, transparent 3%, black 34%, black 100%)` grid-opacity fade is the sole decorative gradient exception; do not add color or background gradients.
