...
**Architecture:** A `ScanSource` seam emits raw strings (keyboard-wedge today, serial via Rust). A **serial `ScanQueue`** processes one scan to completion before starting the next: validate (domain) → journal → signal. The journal write is code row first (only when accepted), then the event row — deliberately NOT one SQLite transaction: `tauri-plugin-sql`'s pooled connections can hand a different connection to every call, making a multi-call `BEGIN`/`COMMIT` unsound. `codes_mirror.code_hash`'s `PRIMARY KEY` is the real duplicate backstop instead — a constraint violation on the code insert **is** the duplicate verdict, reported back as `alreadyPresent` rather than thrown. Serial processing is what makes duplicate detection honest (one scan in flight at a time) and is unrelated to the journal's transaction-free write. Hardware lives behind a TypeScript `HardwareContract` shaped like the idento agent, implemented by Tauri commands now and replaceable by an external agent later.
...
- Produces:
  ...
  - `recordScan(exec: SqlExecutor, e: ScanEventRow, code: AcceptedCode | null): Promise<RecordScanResult>` — writes the code row to `codes_mirror` FIRST (only when `code` is non-null), then always appends the event row SECOND. Deliberately NOT one transaction: `tauri-plugin-sql` opens SQLite through sqlx's pooled connections and can hand a different one to every call, so a multi-call `BEGIN`/`COMMIT`/`ROLLBACK` would not actually be atomic. Instead `codes_mirror.code_hash` being the `PRIMARY KEY` is the real duplicate backstop: a constraint violation on the code insert means this exact code is already accepted and is reported back as `alreadyPresent` (not thrown); any other error from the code insert rethrows. The event row's `verdict` is corrected to `"duplicate"` when the code insert hit `alreadyPresent`, regardless of what the caller passed in, so the journal always records the verdict the operator actually saw
  - `interface RecordScanResult { storedCode: boolean; alreadyPresent: boolean }`
 ...
 * pool are simply unsound. `COMMIT` can fail with "no transaction is
 * active" under real overlapping DB work; the `ROLLBACK` that used to follow
 * would then discard the code row, `recordScan` would throw, and the scan
 * queue's catch-and-log would swallow it — the scan would vanish with no
 * signal to the operator, and the same code could be accepted again later.
 *
