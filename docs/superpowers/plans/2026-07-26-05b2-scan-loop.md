...
- Produces:
  - `loadCodeKeys(exec: SqlExecutor, shiftId: string): Promise<Set<string>>`
...
export function WorkScreen({
  ...
  // The domain's isDuplicate(key) is synchronous, so the shift's accepted keys
  ...
  useEffect(() => {
    let cancelled = false;
    void loadCodeKeys(exec, shiftId).then((loaded) => {
