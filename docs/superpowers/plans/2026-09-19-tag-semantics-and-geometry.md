...
Строки 182–184 заменить на тег фазы, сохранив выражение подписи без изменений:
...
Прежний `glyph={null}` здесь не означал, что статус Честного знака не фаза. Это была обходная заплатка против рассогласования размеров — ровно то, что чинит вся эта работа. Статус в национальной системе — жизненный цикл кода: эмитирован, нанесён, введён в оборот, выведен. Значит по правилу спеки ему положен тег фазы, а не категории.

Плоский тег категории здесь был бы потерей: сегодня шесть состояний различаются цветом (`INTRODUCED` зелёный, `EMITTED` и `APPLIED` синие, три состояния вывода янтарные), и один стальной тег на все шесть стёр бы это различие.
...
## Вторая поправка по ходу исполнения, 19.09.2026

Правило перевода «было — стало», которым задачи 10, 12 и 13 переводят
многострочные тернарные выражения, оказалось жёстче смысла. В задаче 10 по нему
механически перевели четыре ветки вопреки подписям, которые эти состояния носят:

- «Отозвано» у устройства получило `failed` — красную ошибку там, где человек
  сам вывел устройство из оборота. Верно `retired`.
- «Не в сети» получило `none` — «значения нет», хотя состояние есть и сама
  страница складывает его в счётчик требующих внимания. Верно `attention`.
- «Занято» у лицензионного слота получило `running` — вращающуюся стрелку
  незавершённой операции на устоявшемся состоянии владения. Верно `active`.
- «Нужна проверка» получило `failed` вместо буквального `attention`.

Правило остаётся подсказкой, а не таблицей подстановки. Опора — смысл состояния
и его подпись, а не тон, который у него был раньше. Прежний тон говорит лишь о
том, как это выглядело до работы, и ровно это мы и чиним.

Побочно: оба выражения вынесены из JSX в экспортируемые функции
`deviceStatusPhase` и `licenseSlotPhase`, что сделало их проверяемыми тестом.
Там, где тернарная лесенка в разметке мешает закрепить выбор фазы тестом,
выносить её — правильный ход, а не расширение объёма.
