...
1. Примените миграцию базы данных, добавляющую
   `org_profiles.default_box_label_template_id` и выполняющую детерминированный
   backfill. До продолжения проверьте журнал миграций и новую tenant-scoped
   ссылку в целевой базе.
2. Разверните API, который понимает организационный default, требует
...
3. Только после успешных миграции и API rollout разрешите использование нового
   Admin и установку новой Station beta. Admin не должен сохранять новый
   default, а Station не должна создавать или восстанавливать смену через
   новый контракт до готовности API.
...
5. Установите пакет вручную на целевую Windows-станцию и зафиксируйте результаты
   ...
   offline-blocked recovery и upgrade ранее затронутой активной смены. В
   последнем случае на реальном принтере должен напечататься тот же короб и
   SSCC после backfill без повторного закрытия или allocation.
