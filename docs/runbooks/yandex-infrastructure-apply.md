...
2. Запустите **Yandex infrastructure** в `mode=plan` с пустыми `plan_key` и
   `plan_sha256`. Используйте `enable_public_dns=true` только для прямого
   переключения customer admin, SaaS admin, kiosk и landing на app VM; независимо
   задайте текущее одобренное значение `enable_station_release_public_dns`.
3. Approve `production-infrastructure`. Этот run только сохраняет plan в
   защищённом state bucket и выдаёт non-secret `plan_key` и `plan_sha256`.
...
5. После явного подтверждения запустите новый dispatch `mode=apply` с теми же
   target SHA и обоими DNS flags, точным `plan_key` и точным 64-hex
   `plan_sha256`.
