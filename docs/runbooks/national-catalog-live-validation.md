...
5. Для каждой группы ЧЗ явно зафиксируйте решение через
   ...
   принимает одну или несколько проверенных версий схем (до 20 за запрос),
   `ambiguous` — не менее двух непроверенных кандидатов,
   `unmapped` — пустой список. JSON-отчёт следующего шага содержит подходящие
   `categoryIds` и `schemaVersionIds`. Проверка `exact` добавляет или подтверждает
   указанные пары «группа — версия категории», сохраняя прежние пары, их ID и
   исторические версии. Новую категорию можно подтвердить отдельно, не передавая
   уже проверенные категории. Несколько проверенных категорий одной группы не
   считаются неоднозначностью; оставшиеся непроверенные кандидаты требуют review.
   `ambiguous` и `unmapped` по-прежнему заменяют весь набор группы: это явный
   пересмотр, а не способ добавить новую категорию. Этот шаг не активирует схемы
   автоматически.
...
7. Если значения НК должны обновлять `name`, `print_name` или `shelf_life_days`,
   ...
   Пустой список отключает такие переносы для версии. Прямые соответствия
   «Код продукции в ЕГАИС» и «Срок годности, дней» в новом сравнении товаров работают
   независимо от схемы; пользователь подтверждает только изменяемые значения.
   Совпадающие значения видны как «Совпадает», включая фильтр сопоставимых полей,
   и не создают операций записи.
8. Активируйте каждую просмотренную версию через
   `POST /platform/operations/national-catalog/schema-versions/:id/activate`.
   Обновление версии выводит из активных только прежнюю версию той же категории;
   другие категории группы остаются доступны, сохранённые профили не перепривязываются.
   Новые категории обнаруживаются очередным `schema-refresh`, затем проходят те же
   review и activation. Заблокированные схемы не активируйте: устраните конкретную
   причину нормализации и повторите загрузку.
