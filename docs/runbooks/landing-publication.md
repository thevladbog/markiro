...
3. Явно записать состояние уведомления Роскомнадзор: на дату этой редакции оператор сообщил, что **уведомление подано**. Статус уведомления не является секретом; дату подачи и регистрационный идентификатор следует хранить во внешнем compliance evidence. Подача уведомления не означает автоматического внесения в реестр, а репозиторные тесты не подтверждают внешний статус рассмотрения.
