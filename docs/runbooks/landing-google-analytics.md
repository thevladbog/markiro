...
Этот runbook фиксирует минимальную production-конфигурацию веб-аналитики лендинга через общий GTM-контейнер: GA4 и Яндекс Метрику. Он не разрешает рекламные пиксели, remarketing, передачу персональных данных или включение других поставщиков аналитики.
...
Яндекс Метрика и официальный тег подтверждения Яндекс Вебмастера устанавливаются в том же GTM-контейнере. Тег подтверждения при этом остаётся вспомогательным: контейнер загружается только после согласия на аналитику, поэтому робот Яндекса его не видит, и владение сайтом подтверждается через DNS TXT-запись по `docs/runbooks/landing-publication.md`. Метрика, включая Вебвизор, должна требовать `analytics_storage`; поля demo-формы и другие персональные данные нельзя передавать в события или записывать в Вебвизоре. CSP разрешает WebSocket и iframe Метрики, а для тега Вебмастера — только закреплённый `webmaster-verification.js`. При обновлении версии официального шаблона одновременно обновите точный URL в CSP и production-контрактах.
