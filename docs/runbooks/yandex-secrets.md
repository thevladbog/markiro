...
## Офлайн-гранты устройств

Lockbox не сохраняет записи с пустым значением, поэтому до включения подписывания
оставьте следующие четыре записи полностью отсутствующими:
...
В отключённом состоянии `runtime-inventory` разрешает отсутствие только всей
группы. Подписывание включается новой версией `markiro-production-runtime`, где
сохранён текущий payload и одновременно заданы канонический API origin, активный
`kid`, EC P-256 PKCS8 private key и совпадающий публичный keyset. Частичная группа
останавливает deploy на `runtime-inventory`, а несогласованная конфигурация — API
на startup validation. Закрытый ключ нельзя выводить в terminal, workflow logs,
issue, чат или сохранять в Git.

Перед включением подписывания создайте и утвердите версию политики через SaaS
Admin, привяжите её к новой опубликованной версии тарифа и подтвердите, что
устройства получают `observe`. `strict` требует отдельной утверждённой версии
политики с точным списком устройств и отдельной физической приёмки.
