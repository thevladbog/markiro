...
### Онлайн-обновление вариантов документов (0127)

Использовать штатный runtime migration job; локальный `pnpm --filter @markiro/db
db:migrate` также собирает и запускает этот путь. Прямой `drizzle-kit migrate`
или вызов Drizzle `migrate()` не является онлайн-путём для работающей базы.

Runtime сохраняет исходный SQL и хеш `0127_dark_beyonder`, но применяет эквивалент
поэтапно под общей session advisory lock: предшествующие миграции; добавление
колонки и `CHECK NOT VALID`; отдельная валидация; `CREATE UNIQUE INDEX CONCURRENTLY`;
короткая транзакция `UNIQUE USING INDEX`, удаления старого ограничения и записи
в журнал Drizzle. Последующие миграции выполняются только после этого.

Сканирование не держит длительную блокировку записей. Короткие DDL-блокировки
по-прежнему нужны; ожидание блокировки в этой стадии ограничено 5 секундами,
длительность команды — штатным `statement_timeout`. При ошибке не запускать
новую версию приложения: повторить тот же migration job. До успешной финальной
транзакции старое ограничение уникальности остаётся активным. Готовый индекс
переиспользуется; INVALID-индекс после отменённого построения пересоздаётся
конкурентно. Не удалять колонку, старое ограничение или записи журнала вручную.
Исторические строки и ключи файлов не переписываются; уже применённая 0127
пропускается. Изменённый хеш миграции или неожиданный подготовленный индекс
останавливают запуск, а не принимаются за успешную миграцию.
