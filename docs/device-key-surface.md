...
| `integrations`                                                                         | configuring exchange channels, reading their journal, and issuing/rotating a channel's machine credentials (`POST /integrations/:type/credentials`, whose one-time secret response must never reach a device) is a back-office concern; the station never calls this module |
