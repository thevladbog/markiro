...
| PRO-004 | P0  | Add a reproducible synthetic demo tenant and reset.                       | One command creates an identical dataset; a repeated run is idempotent or performs a controlled reset.                   | DEMO    |
...
| LOC-002 | P0  | Store the full Location Description: business name, phone, street/coordinates, city, state/region, ZIP/postal code, country. | The validator requires all fields for an export-ready location snapshot.                               | FDA-05        |
...
| SHP-009 | P1  | CSV/ASN import/export adapter interface; implementation optional before the MVP release.                                                     | Adapter contract exists; P0 may use manual entry and CSV.                                     | FDA-04  |
...
| RQ-003 | P0  | Dry-run validation before export.                                                 | The system shows missing/ambiguous data and blocks export-ready status until resolved or acknowledged. | FDA-06          |
...
| EXP-010 | P0  | The P0 synthetic package generates in < 60 seconds on the development baseline.                                                            | An automated performance test or timed evidence run records the result.                                 | DEMO          |
...
| STN-007 | P1  | Log print requested/confirmed/skipped and printer identity.                            | The evidence export distinguishes the action from a successful physical print.               | DEMO    |
...
| NFR-013 | P1  | Performance: trace query < 2 s for demo; package < 60 s.                                                                                  | A timed acceptance run is saved as evidence.                                                                                                                                                                                                                | DEMO     |
...
| NFR-016 | P0  | Run the U.S. product as a separate deployment and do not host or persist its production infrastructure or data in the Russian Federation. | The infrastructure inventory covers databases, object storage, logs, telemetry payloads, mail, secrets and backups; deployment checks reject Russian regions and endpoints. Remote access from Russia remains allowed under MFA, least privilege and audit. | PRODUCT  |

## EVD — Evidence and MVP package

| ID      | P   | Requirement                                                                                              | Acceptance                                                                                                 | Basis    |
| ------- | --- | -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | -------- |
| EVD-001 | P0  | Tagged public MVP release.                                                                               | Release tag, commit SHA, changelog and installer/web build hashes preserved.                               | EVIDENCE |
...
| EVD-009 | P1  | Obtain review from a U.S. food-safety/traceability specialist.                                           | Dated written feedback or a signed review memo addresses scope and terminology.                            | DEMO     |
| EVD-010 | P1  | Obtain structured product feedback from 1–2 U.S. food-industry practitioners after a real demonstration. | Feedback records the reviewer's own problem, observations and permission to cite it.                       | DEMO     |
...
| EVD-012 | P0  | Maintain a source-to-claim register for public product statements.                                       | Every product statement cites a release, test, screenshot, generated artifact or external feedback record. | DEMO     |
