...
- Do not fork the product. Add a `traceability` bounded context and run it in a
  dedicated U.S. deployment.
- Do not host or persist U.S. production data, logs, telemetry payloads, mail,
  secrets or backups in the Russian Federation. Remote access may originate
  there only through least-privilege, multi-factor authentication and audited
  access; never copy production data into Russian infrastructure, fixtures or
  CI artifacts.
