...
Receiving void rule approved 2026-09-07: preserve lot ID, product, TLC, source, assignment basis, operational status and the permanent source lock. Do not automatically archive, quarantine, release or reopen lots. Track current receiving basis separately: another current finalized Receiving referencing the same lot can retain that basis; without one, show the absence explicitly. Future Transformation/Shipping must validate the applicable origin rather than treating `active` as receipt evidence. A manually created lot is not itself a Receiving. The [Receiving lifecycle design](../superpowers/specs/2026-09-07-us-03-receiving-lifecycle-design.md) and its technical choices were approved by the owner on 2026-09-07. Rules/contracts, additive storage, internal authorized lifecycle commands and current support/history/registry reads are implemented. Coordinated HTTP integration and connected UI remain pending. Internal server proof does not establish downstream acceptance.
