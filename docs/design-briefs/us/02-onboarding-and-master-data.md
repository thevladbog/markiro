...
- **Readiness is a property of the record.** A location is "case-ready" (all seven Location
...
For `RU_CHZ` the sidebar is pixel-identical to today — draw it beside the U.S. one as the
control. A user whose only capability is `traceability.read` (a Receiving Operator) lands on
`/traceability`, never on a forbidden page.

**Header badge.** Next to the organization name: `US · FSMA 204` or `US · Generic`; nothing for
RU. Text, not a colored dot — the PRO-005 reminder of the regime on every screen.
...
timezone → processor location case-ready → supplier and recipient → products reviewed → first
receiving) — this brief's proposal, not the spec's.

States to draw: RU control; FSMA fresh (checklist, empty tiles); FSMA populated (North River
numbers); generic (statement, no CTE block); loading; profile failed to load (retry); stale.

### 2. Profile settings

A **Regulatory profile** card at the top of Settings: profile select with one-line descriptions
(`RU_CHZ` "Chestny ZNAK marking (current)"; `US_FSMA204_PROCESSOR` "U.S. processor — FTL foods,
Receiving / Transformation / Shipping"; `US_GENERIC_LOT_TRACEABILITY` "U.S. generic lot
traceability — not classified as FTR-covered"); baseline ID and verified date, read-only, set
by the server on any U.S. transition and empty for RU; timezone select that swaps to the U.S.
list (New York, Chicago, Denver, Phoenix, Los Angeles, Anchorage, Honolulu, Puerto Rico) when a
U.S. code is chosen and becomes required (`time_zone_required`), with the hint that every date
on every record is shown in this zone; retention days, default 1825, floor 730 explained ("kept
at least 2 years; default 5"), `retention_below_minimum` as a field error; a confirm dialog on
code change listing what will visibly change (RU sections hidden, Traceability section shown,
timezone written).
...
States to draw: RU active (card present, nothing else changed); choosing a U.S. code (timezone
list swaps, confirm dialog); FSMA active; generic active; locked; read-only subscription; error.
...
| Location Description (required for case-ready) | Business name (prefilled from party, editable); phone; address kind radio: street address **or** coordinates (lat / long); city; state code; ZIP; country (ISO select, default US) |
...
text. In the FSMA profile the seven description fields carry "Required for case-ready" and API
...
list, card and picker: "Case-ready" (check icon) or "Incomplete — missing: phone, ZIP" (warning).
...
variant; incomplete vs case-ready in list and picker; same-address alert; archive confirm;
...
- **KDE groups on forms.** Location Description, Product Description and Lot (TLC + source) are
  ...
  with two textual markers: "Required for case-ready" (seven location fields, packaging size /
