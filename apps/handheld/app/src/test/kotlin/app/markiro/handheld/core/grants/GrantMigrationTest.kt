...
class GrantMigrationTest {
    @Test fun realVersionThirteenUpgradePreservesQueuedBytesAndHasNoImplicitStrictConfig() = runTest {
        ...
        fun database()=Room.databaseBuilder(context,HandheldDatabase::class.java,name).allowMainThreadQueries().addMigrations(MIGRATION_13_14).build()
        try {
            val old=database()
            ...
            listOf("grant_state","grant_tokens","grant_counters","grant_evidence","grant_task_bindings","grant_task_provenance").forEach { sql.execSQL("DROP TABLE $it") }
