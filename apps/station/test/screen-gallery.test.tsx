...
describe("development screen gallery", () => {
  ...
  it.each([
    ...
    [
      "validation-print-create-required",
      "Оператор должен отсканировать новую этикетку перед следующей единицей.",
    ],
    [
      "validation-print-create-none",
      "После отправки на принтер можно сканировать следующую единицу.",
    ],
    ["validation-print-create-template", "Внешняя этикетка"],
    ["validation-print-required", "Отсканируйте напечатанную этикетку"],
    ["validation-print-skipped", "Проверка пропущена"],
    ["validation-print-none", "Отправлено на принтер"],
    ["validation-print-verified", "Этикетка подтверждена"],
    ["validation-print-unknown", "Печать требует уточнения"],
    ["validation-print-reason", "Укажите причину повторной печати"],
    ["validation-print-mismatch", "Код не совпадает"],
