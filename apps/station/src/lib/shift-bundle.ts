...
import { syncStationProductImage } from "./product-image-cache.js";
...
export function mirrorShiftBundle(
  ...
  const operation = (async () => {
    try {
      ...
      if (client.download) void syncStationProductImage(exec, { download: client.download }, bundle.product);
    } catch (err) {
