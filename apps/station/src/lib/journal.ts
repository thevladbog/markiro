...
export interface ScanEventRow {
  /** Stable productive identity for offline-grant replay. Generated by the owner when omitted. */
  eventId?: string;
  shiftId: string;
...
export class OfflineGrantDeniedError extends Error {
  constructor(readonly reason: string) {
    super(`offline grant denied: ${reason}`);
  }
}
...
async function recordScanLegacy(
  exec: SqlExecutor,
  e: ScanEventRow,
  code: AcceptedCode | null,
...
/** Legacy owner entry point. Current grant state may never be bypassed by an older caller. */
export async function recordScan(
  ...
  const [activeGrant] = await exec.all<{ active: number }>(
    "SELECT 1 AS active FROM offline_grant_install_state WHERE id=1",
  );
  if (activeGrant?.active === 1) throw new OfflineGrantDeniedError("grant_aware_owner_required");
  return recordScanLegacy(exec, e, code);
}

/** Runtime owner path: admission, charge, accepted code, audit event and outbox share one native transaction. */
export async function recordScanWithOfflineGrant(
  exec: SqlExecutor,
  e: ScanEventRow,
  code: AcceptedCode | null,
  generation: CredentialGeneration,
  sampleClock: () => Promise<GrantClockSample> = sampleGrantClock,
): Promise<RecordScanResult> {
  ...
  try {
    ...
    if (!(await stationOperatorIsCurrentlyActive(exec, e.operatorId)))
      throw new OfflineGrantDeniedError("operator_unauthorized");
    const [binding] = await exec.all<{ snapshot_digest: string }>(
      `SELECT json_extract(grant_json,'$.snapshotDigest') AS snapshot_digest
    ...
    const decision = await new StationGrantAdmission(exec, sampleClock).commitCompletion({
      ...
      facts: { units: code && e.verdict === "ok" ? 1 : 0 },
      result: { storedCode: false, alreadyPresent: false },
      wrapCommand(command) {
        const grantCommand = JSON.parse(String(command.values[1])) as unknown;
        return {
          sql: "INSERT INTO offline_grant_scan_commands(event_id,payload_json) VALUES(?,?)",
          values: [
            eventId,
            JSON.stringify({
              grantCommand,
    ...
    if (!decision.decision.allow)
      throw new OfflineGrantDeniedError(decision.decision.reason ?? "denied");
    return decision.result as RecordScanResult;
  } finally {
    lease.release();
  }
}
