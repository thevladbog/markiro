...
export async function closeCurrentPallet(
  ...
  if (!closed) {
    ...
    // read above and this UPDATE -- the pooled-connection race
    // `apps/station/AGENTS.md` warns about, or a genuine double close.
    // The serial burned above cannot be un-burned and never reached a
    // stored record, so it is accepted as lost -- exactly the trade
    // `invalid-serial` already names above -- rather than fabricating a
    // `closed` result carrying an SSCC the database never stored.
    return { status: "invalid-serial" };
  }
