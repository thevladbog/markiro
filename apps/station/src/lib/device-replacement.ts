...
> {
  try {
    const [row] = await exec.all<{
      ...
      (SELECT COUNT(*) FROM boxes_mirror WHERE disassembled_at IS NULL AND (acked_at IS NULL OR (closed_at IS NOT NULL AND sscc IS NOT NULL AND confirmed_revision<reconciliation_revision)))+(SELECT COUNT(*) FROM pallets_mirror WHERE acked_at IS NULL AND disassembled_at IS NULL)+(SELECT COUNT(*) FROM inventory_repack_boxes_mirror WHERE state='open') boxes,
