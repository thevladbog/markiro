...
describe.skipIf(!ready)("station-scans e2e", () => {
  ...
  describe("box membership (Task 10)", () => {
    ...
    it("accepts a late-arriving printVerifiedAt for a box the closure already closed earlier", async () => {
      ...
      expect(box!.printVerifiedAt).toBeNull();
      const registryVersion = box!.updatedAt;
      ...
      expect(box!.closedAt?.toISOString()).toBe(ISO);
      expect(box!.sscc).toBe(SSCC);
      expect(box!.updatedAt.getTime()).toBe(registryVersion.getTime());
    });
    ...
    it("advances only the membership displaced by duplicate reconciliation", async () => {
      ...
      const before = new Map([
        [b1Id, await boxUpdatedAt(b1Id)],
        [b2Id, await boxUpdatedAt(b2Id)],
      ]);
      const versionBefore = (boxId: string): Date => {
      ...
      expect((await boxUpdatedAt(displaced.boxId)).getTime()).toBeGreaterThan(
        versionBefore(displaced.boxId).getTime(),
      );
      expect((await boxUpdatedAt(active.boxId)).getTime()).toBe(
        versionBefore(active.boxId).getTime(),
      );
      expect((await registryOwner(tenantId, boxedCodeHash("aa")))?.scannedAt.toISOString()).toBe(
        "2026-07-29T10:00:00.000Z",
      );
