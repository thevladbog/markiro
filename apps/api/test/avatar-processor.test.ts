...
describe("processAvatar", () => {
  it.each(["jpeg", "png", "webp"] as const)("normalizes %s to metadata-free 512px WebP", async (format) => {
    let image = sharp({
  ...
  it("rejects unsupported content even when the MIME claim would say image", async () => {
    await expect(processAvatar(Buffer.from("<svg><script>alert(1)</script></svg>"))).rejects.toThrow(
      /JPEG, PNG, or WebP/,
    );
  });
