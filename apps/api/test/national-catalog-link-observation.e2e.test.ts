...
if (!process.env.DATABASE_URL) throw new Error("Requires isolated local PostgreSQL");
const databaseUrl = new URL(process.env.DATABASE_URL);
if (
  !["localhost", "127.0.0.1"].includes(databaseUrl.hostname) ||
  databaseUrl.pathname !== "/markiro_nc_import_01a08239"
)
  throw new Error("Task10 tests require the exact isolated local import database");
...
it.each([false, true])(
  ...
  async (failed) => {
    ...
    expect(audits[0]).toMatchObject({
      ...
      targetType: "product",
      outcome: "failure",
      after: { linkId, phase: "card", actorKind: "manual", attempts: 1, reason: "access_changed" },
    });
  },
);
...
it.each(["same-url-new-bytes", "new-url-same-bytes", "download-fails", "ambiguous", "stale-photo"])(
  "observes normalized photo bytes with durable card/photo steps: %s",
  async (mode) => {
...
it("keeps a whole legacy-null review unknown after a successful new source observation", async () => {
  ...
  await service.resume(tenantId, linkId);
  expect(
    await new NationalCatalogLinkService(db, authorization, entitlements).read(tenantId, productId),
