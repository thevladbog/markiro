...
describe.skipIf(!ready)("pickup orders admin e2e", () => {
  ...
  it("creates orders via the kiosk path, lists/filters/details/resolves/cancels them as admin", async () => {
    ...
    await db.insert(schema.pickupOrderReasons).values({
      id: writeoffReasonId, tenantId, name: "Брак", sortOrder: 0, archived: false,
    });
    const orderWriteoff = await scan(104, `01${GTIN}21WOFF1${GS}93Abcd`, {
      reason: "writeoff", writeoffReasonId,
    }).expect(201);
    ...
    await agent.post(`/pickup-orders/${idList}/resolve`).send({ action: "writeoff", actNo: "ACT-X" }).expect(400);
    ...
    const [punchRow] = await db.select().from(schema.pickupOrders).where(eq(schema.pickupOrders.id, idPunch));
    ...
    await agent.post(`/pickup-orders/${idPunch}/resolve`).send({ action: "punch", receiptNo: "R-2" }).expect(409);
    ...
    const voidedItems = await db
      ...
      .where(and(eq(schema.pickupOrderItems.tenantId, tenantId), eq(schema.pickupOrderItems.orderId, idCancel)));
