...
type QueryCall = { table: unknown; where: unknown[] };
...
describe("TenantBillingReadService", () => {
  ...
  it("scopes every entity detail and download lookup to its tenant", async () => {
        ...
        return [
          {
            ...
            invoiceId,
            status: "ready",
            objectKey: "tenant/invoice",
          },
        ];
      }
      if (table === schema.commercialOfferDocuments) {
        return [
          { id: offerDocumentId, tenantId, offerId, status: "ready", objectKey: "tenant/offer" },
        ];
      }
      if (table === schema.billingActs) return [{ id: actId, tenantId, status: "issued" }];
      if (table === schema.billingActDocuments) {
        return [{ id: actDocumentId, tenantId, actId, objectKey: "tenant/act" }];
      }
      return [];
    });
    ...
    expect(storage.presignRead).toHaveBeenCalledWith("tenant/invoice", 300);
    expect(storage.presignRead).toHaveBeenCalledWith("tenant/offer", 300);
    expect(storage.presignRead).toHaveBeenCalledWith("tenant/act", 300);
