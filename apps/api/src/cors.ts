...
import { kioskAllowedOrigins, sessionAllowedOrigins, type Env } from "./env";
...
/**
 * Per-route CORS policy: the kiosk origin is trusted on `/kiosk/*` only.
 *
 ...
 * here, not per call.
 *
 * `allowedHeaders` is left unset on both, on purpose: `cors` then echoes
 * whatever `Access-Control-Request-Headers` asked for, which is how
 * `x-kiosk-token` clears preflight without being named here.
 */
export function corsDelegate(env: Env): CorsOptionsDelegate<Request> {
  ...
  return (req, cb) => cb(null, isKioskPath(req.path) ? kiosk : session);
}
