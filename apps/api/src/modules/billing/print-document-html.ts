...
const authorizedSignatureDataUri = `data:image/png;base64,${readFileSync(
  join(__dirname, "assets/vb-signature-ink.png"),
).toString("base64")}`;
const legalSealDataUri = `data:image/png;base64,${readFileSync(
...
const documentHeader = (model: PrintDocumentModel, firstPage = false) => {
  ...
  if (model.kind === "offer" && model.status === "draft")
    return `<header class="document-header${firstPage ? " print-first" : ""}"${firstPage ? ' aria-hidden="true"' : ""}>${logo}<div class="document-id"><strong>Черновик. Не выпущен</strong><span>Номер и дата выпуска не присвоены</span></div></header>`;
...
const documentFooter = (model: PrintDocumentModel, barcode: string, firstPage = false) => {
  if (model.kind === "offer" && model.status === "draft")
    return `<footer class="document-footer${firstPage ? " print-first" : ""}"${firstPage ? ' aria-hidden="true"' : ""}>Черновик. Не выпущен · Сформировано системой Markiro</footer>`;
...
const serviceUsageTable = (model: PrintDocumentModel) => {
  const entries = model.serviceUsage ?? [];
  return entries.length === 0
    ? ""
    : `<div class="items-heading"><span>ВЫПОЛНЕННЫЕ РАБОТЫ</span><span class="mono muted">${entries.length} записей</span></div><table class="items-table"><thead><tr><th>№</th><th>РАБОТА</th><th>ФАКТ</th><th>ИЗ ПАКЕТА</th></tr></thead><tbody>${entries
        .map(
          (entry) =>
            `<tr><td class="mono number">${entry.sequence}</td><td><strong>${escape(entry.workReference)}</strong><small>${escape(entry.description)} · ${entry.kind === "correction" ? "корректировка" : "работа"} · ${entry.classification === "product_defect" ? "не списывается из пакета" : "работа из пакета"}</small></td><td class="mono numeric">${entry.actualMinutes} мин</td><td class="mono numeric">${entry.allowanceMinutes} мин</td></tr>`,
        )
        .join("")}</tbody></table>`;
};
...
const closing = (model: PrintDocumentModel, signed: boolean) => {
  const terms =
  ...
  const provider = signatureBlock(model.kind === "act" ? "ИСПОЛНИТЕЛЬ" : "ПОСТАВЩИК", signed);
  const counterpart =
    model.kind === "act"
      ? signatureBlock("ЗАКАЗЧИК", false)
      : signed
        ? ""
        : '<div class="stamp"><span>МЕСТО ДЛЯ ПЕЧАТИ</span></div>';
  return `${totals(model)}${terms}${payment}<section class="signing${signed ? " signing--signed" : ""}">${provider}${counterpart}</section>`;
};
