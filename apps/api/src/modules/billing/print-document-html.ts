...
const documentHeader = (model: PrintDocumentModel, page: number, totalPages: number) =>
  `<header class="document-header">${markiroLogo}<div class="document-id"><strong>${documentKindLabel(model)}</strong><span>№ ${escape(model.number)} · ${formatPrintDate(model.issuedOrPublishedAt)}${totalPages > 1 ? ` · Лист ${page} из ${totalPages}` : ""}</span></div></header>`;
