...
import type { readReceivingReferenceContext } from "./us-receiving-reference-context";
...
export function planReceivingSnapshot(
  ...
  context: Context,
  review: { actorUserId: string; finalizedAt: string; reviewedExemptLines: number[] },
  ...
  const planned = draft.items.map((line, index) => {
    const assessment = assessReceivingExemptionLine(line, draft.locationId);
    ...
    return {
      ...
      lotId: line.lotLinkMode === "create_on_finalize" ? randomUUID() : line.lotId,
