  ...
  private async billingSourceIsValid(
    ...
    if (eventKind === "clarification_required") {
      ...
      query = [
        ...
        "    AND event.id = (",
        "      SELECT latest_relevant.id FROM tenant_billing_request_events AS latest_relevant",
        "      WHERE latest_relevant.tenant_id = request.tenant_id",
        "        AND latest_relevant.request_id = request.id",
        "        AND latest_relevant.kind IN ('status_changed', 'tenant_reply')",
        "      ORDER BY latest_relevant.created_at DESC, latest_relevant.id DESC",
        "      LIMIT 1",
        "    )",
        ") AS valid",
      ].join("\n");
