...
export class IntegrationsController {
  ...
  @ApiOperation({
    ...
    description:
      ...
      "for in the last day, and how many carry no ЧЗ product group -- those are stuck until " +
      "their product gets one, since `cises/info` takes the group as a query parameter.",
  })
  @ApiParam({ name: "type", enum: INTEGRATION_CHANNEL_TYPES })
  ...
  //
  // 204, а не дефолтный 200: тела у ответа нет, и статус говорит это явно.
  // (`apiFetch` в админке теперь переживает и пустое тело с 200 -- см.
  // apps/admin/src/api/client.ts, -- но 204 остаётся честным контрактом, а
  // не полагается на толерантность клиента.)
  @Delete(":type")
