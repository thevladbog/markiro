...
<body>
  <main class="page${compact ? " compact" : ""}" data-layout="${compact ? "compact" : "standard"}">
    ...
    <section class="product${data.imageChecksum === null ? " product--no-photo" : ""}">${photo}<div><div class="product-eyebrow">Продукция смены</div><div class="product-name">${product}</div><div class="product-meta">${printNameSegment}GTIN <span>${data.gtin14}</span></div></div></section>
