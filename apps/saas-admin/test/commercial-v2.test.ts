...
import { TENANT_DETAIL, DRAFT_PLAN, ADDON, SERVICE } from "./render.js";
describe("current commercial fixtures", () => {
  it("represents tenant purchased periods and zero-capable catalog responses", () => {
    platformTenantV2Contracts.detail.response.parse(TENANT_DETAIL);
    expect(
      platformCatalogV2Contracts.list.response.safeParse({ items: [DRAFT_PLAN, ADDON, SERVICE] }),
...
it("negotiates V2 across catalog tenant and seller consumers and sends the reviewed publish identity", async () => {
  ...
  expect(
    calls.every((call) => new Headers(call.headers).get("X-Markiro-Commercial-Version") === "2"),
  ).toBe(true);
