    ...
    <>
      ...
      <Route path="/" element={<ShellPage />}>
        ...
        <Route path="km-orders">
          ...
          />
          {/* The order card reads the same orders the list does, so it is
              gated on the read capability like its sibling; issuing codes from
              it is gated again, inside the page, on the write capability. */}
          <Route
            path=":orderId"
            element={
              <RequireCapability capability={C.OPERATIONS_READ}>
