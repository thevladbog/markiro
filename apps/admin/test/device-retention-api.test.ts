...
import {
  ...
  retentionErrorKind,
} from "../src/pages/devices/device-retention-api.js";
import {
  inspection,
  preview,
  request,
...
it("validates routes, bodies and original receipt", async () => {
  ...
  expect(fetch.mock.calls.map((x) => x[0])).toEqual([
    "/api/device-licensing/retention",
    "/api/device-licensing/retention/preview",
    "/api/device-licensing/retention/confirm",
  ]);
...
it.each([
  ...
  vi.stubGlobal("fetch", vi.fn().mockResolvedValue(response({ code }, 409)));
  expect(retentionErrorKind(await inspectDeviceRetention("tenant-a").catch((e) => e))).toBe(
    "conflict",
  );
});
it.each([401, 403])("recognizes complete authorization %i", async (status) => {
  vi.stubGlobal(
    ...
    vi
      ...
      .mockResolvedValue(
        response(
          { statusCode: status, message: status === 401 ? "Unauthorized" : "Forbidden" },
          status,
        ),
      ),
  );
