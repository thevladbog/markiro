...
import {
  ...
  type PrepareAttempt,
  type CancelAttempt,
} from "../src/pages/devices/replacement-state.js";
...
it("closes known authorization denial and refreshes current permissions", async () => {
  fail = (url) =>
    url.endsWith("/confirm") ? response({ message: "Forbidden", statusCode: 403 }, 403) : undefined;
  setup();
  await inspect();
  await userEvent.click(screen.getByRole("button", { name: "Save preparation" }));
...
it("requires fresh preview and explicit confirmation after known conflict", async () => {
  fail = (url) =>
    url.endsWith("/confirm") ? response({ code: "device_replacement_stale" }, 409) : undefined;
  setup();
  await inspect();
  await userEvent.click(screen.getByRole("button", { name: "Save preparation" }));
