...
import { DeviceReplacementPanel } from "../src/pages/devices/DeviceReplacementPanel.js";
import {
  pool,
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (failure === "upgrade") {
        ...
        return response({ code: "client_upgrade_required" }, 409);
      }
      if (failure === "auth") {
        failure = null;
        writable = false;
        return response({ statusCode: 403, message: "Forbidden" }, 403);
      }
      ...
      if (url.endsWith("/pairing-code"))
        return response({ code: "12345678", expiresAt: "2026-09-18T13:00:00.000Z" });
