...
async function scanPackage(path) {
  const manifestBytes = await readFile(join(path, "package.json"));
...
async function scanTask(task) {
  ...
  for (const entry of await entries(path)) {
    ...
    if (task.kind === "nodeModules") {
      if (entry.name === ".pnpm") enqueue("store", child);
      else if (entry.name === ".bin" || entry.name.startsWith(".")) continue;
      else if (entry.name.startsWith("@")) enqueue("scope", child);
      else enqueue("package", child);
    } else if (task.kind === "scope") {
      if (!entry.name.startsWith(".")) enqueue("package", child);
    } else if (task.kind === "store") {
      if (entry.name !== "node_modules" && !entry.name.startsWith("."))
        enqueue("storeEntry", child);
    }
  }
}
